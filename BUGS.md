...
## BUG-054 · Portfolio scan: raw_html stored as NULL instead of empty string
...
**Root cause:** The INSERT used the SQL literal `NULL` for `raw_html`. Screenshot-based snapshots have no HTML source — the column should default to `''` (consistent with other non-HTML snapshot paths).

**Fix:** Changed `NULL` → `''` in the INSERT VALUES clause for `raw_html`.

**Regression test:** `tests/api/portfolio-scan.test.ts` — "BUG-054: stores raw_html as empty string, not NULL"

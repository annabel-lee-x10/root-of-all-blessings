          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '10px', marginBottom: '1rem' }}>
              <div style={card}>
                ...
                <div style={{ ...valueStyle, color: loading ? 'var(--text-muted)' : '#f85149' }}>
                  {loading ? '...' : fmt(data?.total_spend ?? 0)}
                </div>
                <div style={{ color: 'var(--text-muted)', fontSize: '11px', marginTop: '2px' }}>SGD</div>
              </div>

              <div style={card}>
                <div style={labelStyle}>Income</div>
                <div style={{ ...valueStyle, color: loading ? 'var(--text-muted)' : '#3fb884' }}>
              ...
              <div style={card}>
                ...
                <div style={{ ...valueStyle, color: loading ? 'var(--text-muted)' : 'var(--text)' }}>
                  {loading ? '...' : fmt(data?.daily_average ?? 0)}
                </div>
                <div style={{ color: 'var(--text-muted)', fontSize: '11px', marginTop: '2px' }}>SGD / day</div>
              </div>

              <div style={card}>
                <div style={labelStyle}>Budget</div>
                <div style={{ ...valueStyle, color: 'var(--text-muted)', fontSize: '18px' }}>
                  {loading ? '...' : (data?.budget_remaining != null ? fmt(data.budget_remaining) : '\u2014')}
                </div>
                <div style={{ color: 'var(--text-muted)', fontSize: '11px', marginTop: '2px' }}>not configured</div>
              </div>
            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {data.category_breakdown.slice(0, 6).map((cat) => (
                    <div key={cat.category_name} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <span style={{ color: 'var(--text-muted)', fontSize: '11px', minWidth: '38px', textAlign: 'right' }}>
                        {cat.pct.toFixed(1)}%
                      </span>
                    </div>
                  ))}
